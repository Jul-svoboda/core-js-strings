...
function isPalindrome(str) {
  if (
    str.toLocaleLowerCase().replace(/[^a-za-яё]/g, '') ===
    str
      .toLocaleLowerCase()
      .replace(/[^a-za-яё]/g, '')
      .split('')
...
function findLongestWord(sentence) {
  const strArry = sentence.split(' ');
  let maxStr = strArry[0];
  for (let i = 0; i < strArry.length - 1; i += 1) {
    if (maxStr.length < strArry[i + 1].length) {
      maxStr = strArry[i + 1];
    }
  }
  return maxStr;
...
function reverseWords(str) {
  ...
  let newStr = '';
  for (let i = 0; i < strArry.length; i += 1) {
    newStr += `${strArry[i].split('').reverse().join('')} `;
  }
...
function extractNameFromTemplate(value) {
  ...
  return `${newArry[1]} ${newArry[2].slice(0, -1)}`;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < allWords.length; i += 1) {
    const allChars = allWords[i].split('');
    for (let j = 0; j < allChars.length; j += 1) {
      const code = allChars[j].charCodeAt(0);
      if (code >= 65 && code <= 90) {
        if (code + 13 > 90) {
          allChars[j] = String.fromCharCode(code - 13);
        } else {
          allChars[j] = String.fromCharCode(code + 13);
        }
